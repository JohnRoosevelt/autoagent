...
- ☑ local retriever 使用归一化 token overlap，稳定 score/id 排序并限制结果数量；
- ☑ 不调用 embedding API、不使用数据库或远端向量服务；
- ☑ 完整 Rust 质量门禁通过。
...
- ☑ 出站 guardrail 标记常见 instruction-like 外部内容并 redaction 显式配置的密钥文本；
- ☑ 不执行或遵从外部内容，也不记录未过滤 secret；
- ☑ 完整 Rust 质量门禁通过。

## 第 33 章 Checkpoint

- ☑ 为调用方选择的稳定消息前缀生成确定性本地 fingerprint；
- ☑ 依据明确 pricing 计算 input、cached input 与 output token 成本；
- ☑ 不发送 provider cache-control、不声称命中远端缓存；
- ☑ 完整 Rust 质量门禁通过。
...
- ☑ 默认拒绝文件写入、删除、命令执行和网络访问；
- ☑ 仅显式授予的 capability 可通过审批；不执行操作；
- ☑ `cargo fmt --check`、`cargo check`、`cargo clippy -- -D warnings`、`cargo test` 全部通过。
...
- ☑ `SandboxProfile` 声明只读、禁网的受限边界；
- ☑ 所有 untrusted execution 明确拒绝，未运行进程、容器或代码；
- ☑ `cargo fmt --check`、`cargo check`、`cargo clippy -- -D warnings`、`cargo test` 全部通过。
...
- ☑ 本地 `TraceRecorder` 以稳定顺序记录结构化 span；
- ☑ 不导出 trace、不接入日志/指标后端；
- ☑ `cargo fmt --check`、`cargo check`、`cargo clippy -- -D warnings`、`cargo test` 全部通过。
