...
## 第 29 章 Checkpoint（已发布为 `chapter-29-evaluation`）
...
- ☑ local deterministic token-overlap retrieval；无 embedding API、vector DB 或自动 context injection。

## 第 35 章 Checkpoint（已发布为 `chapter-35-web-fetch`）

- ☑ strict HTTPS exact-host allowlist、无 credentials/ports、size cap 和 local HTML text；无真实 GET。
...
- ☑ 有界 inbox 按顺序在宿主安全回合边界应用插话；
- ☑ 取消复用协作式 token，即使 inbox 满也立即标记；不抢占同步工具。
